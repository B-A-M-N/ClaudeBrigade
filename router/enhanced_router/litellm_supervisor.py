...
class LiteLLMSupervisor:
    ...
    async def reload(
        ...
        result = await self.start_generation(
            registry_hash=registry_hash,
            models=models,
    ...
    async def _drain_old(
        ...
        while asyncio.get_event_loop().time() < deadline:
            if old_gen is not None:
                remaining = self._state.count_active_bindings_for_generation(old_gen)
            else:
                remaining = 0
            ...
            if not warned:
                LOGGER.info(
                    "waiting for %s active binding(s) on gen=%s to release",
                    remaining,
                    old_gen,
                )
                warned = True
            ...
            LOGGER.warning(
                "drain timed out for gen=%s after %.1fs, force-killing",
                old_gen,
                drain_timeout,
            )
